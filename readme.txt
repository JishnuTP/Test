
Task - details

https://github.com/JishnuTP/Test.git

clone the link.

test portal

front-end---- side in use vite , react.js and tailwind.css and mui
backend - node.js , express , mongoDB ,JWT

To run this in locally- change the base URL to localhost


you can find two folder inside this link just change directory to "Frontend "--- this  is front

to start frontend -  npm i --- to  install node modules and then "npm run dev".

to start backend -  npm i --- to  install node modules and then "node index.js". (or nodemon index.js)




instruction to use

after start front end it navigate to a landing page with button go to .

navigate - login/registration 

for review purpose i setup a option in registration section to select admin or user if don't select anything register as user.


there are two type of user login  1- user 
2-admin

when user login its redirect to same page with logged credentials and some option like start test and result option appears and admin can have option to add test and publish result .

when admin publish result its shows in result section of user.
 